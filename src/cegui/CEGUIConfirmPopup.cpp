...
CEGUIConfirmPopup::CEGUIConfirmPopup(const string& name, const string& title):
    ConfirmPopup(name, title),
    m_message(NULL)
{
}

void CEGUIConfirmPopup::Init ()
{
    orxASSERT(
    ...
    window->subscribeEvent (FrameWindow::EventCloseClicked,
	    Event::Subscriber (&CEGUIConfirmPopup::OnCloseClicked, this));
    ...
    m_message = FindTextbox("ConfirmPopup/DynamicMessage");
}

void CEGUIConfirmPopup::SetText(const string& message) {
    m_message->SetText(message);
}

bool CEGUIConfirmPopup::OnCloseClicked (const CEGUI::EventArgs &e)
{
    OnClose();
